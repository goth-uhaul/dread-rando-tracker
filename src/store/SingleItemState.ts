...
export class SingleItemState extends BaseItemState {
  ...
  @computed get startedWithThisItem(): boolean {
    return this.startingLocation === locations.Starting;
  }
  ...
  decrementUpgradeCount() {
    this.upgradeCount =
      this.upgradeCount <= 0 ? this.item.maxUpgrades : this.upgradeCount - 1;
  }
}
